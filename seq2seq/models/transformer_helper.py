...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        Padding is only used to ensure that all sequences have the same length. This padding, however, does not carry 
        meaningful information and should not influence the model. The encoder_padding_mask is used to prevent the model
        from attending to padding tokens when computing self-attention weights.
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        Encoder attention is one cross attention. It allows each position in the decoder to attend to all positions in 
        the encoder's output. The encoder outputs provide the keys and values. The decoder state provide the query to
        access the entire input sequence's information. Then, it can get the relation between decoder input and encoder 
        output. But in self-attention, each position in a sequence attend to all positions within the same sequence.
        The attention obtained is the correlation of different tokens in the same sequence.
        
        2.  What is the difference between key_padding_mask and attn_mask? 
        key_padding_mask is used specifically to prevent the attention mechanism from attending to padding tokens within
        the input sequence. This mask ensures that the model does not consider these positions during attention weight 
        calculation, thereby focusing only on the meaningful part of the input data. However, attn_mask is used so that
        the model cannot look ahead of the sequence (seeing future words) when making predictions.
        
        3.  If you understand this difference, then why don't we need to give attn_mask here?
        We do not need give attn_mask here because the previous self attention layer (first self attention layer) in the
        decoder already used it. The query computed by the previous attention already avoids attend to the future words.
        '''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # All have size [tgt_time_steps, batch_size, embed_dim]
        q = self.q_proj(query)
        k = self.k_proj(key)
        v = self.v_proj(value)
        ...
        # split by the head
        # all have size [tgt_time_steps, batch_size, num_heads, head_embed_size]
        ...
        v = v.transpose(0, 2)
        # [num_heads*batch_size,tgt_time_steps,head_embed_size]
        q = q.contiguous().view(self.num_heads * batch_size, -1, self.head_embed_size)
